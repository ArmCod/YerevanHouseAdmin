...
import { ConditionPut, getConditionThunk } from "../../store/actions/conditionAction";
...
const ConditionComponent = () => {
    ...
    const condition = useSelector(state => state.conditionReducer.condition[0])
    const [data, setData] = useState()

    useEffect(() => {
        dispatch(getConditionThunk())
    }, [])
    useEffect(() => {
        setData(condition)
    }, [condition]);
    ...
    return (
        <div className="container">
            ...
            <Box m={3} >
                <h2 mt={3} mb={3}>
                    Պայմաններ և կանոններ ֊ կարգավորումներ
                </h2>
