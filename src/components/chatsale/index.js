...
const ChatSaleComponent = () => {
  const [messagesData, setMessagesData] = useState([]);

  useEffect(() => {
    fetch(`https://back.yerevanhouse.net/api/admin/chat/sale`)
      .then((res) => res.json())
      .then((res) => setMessagesData(res));
  }, []);

  const handleDeleteMessage = (id) => {
    fetch(`https://back.yerevanhouse.net/api/admin/chat/sale/drop?id=${id}`, {
      method: "DELETE",
    })
      .then((response) => {
        if (!response.ok) {
...
export default ChatSaleComponent;
