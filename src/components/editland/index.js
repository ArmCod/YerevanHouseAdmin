...
import { getCurrentLand, LandDelete } from '../../store/actions/landAction';
...
const EditLand = () => {
    ...
    useEffect(() => {
        window.scrollTo(0, 0)
        dispatch(getCurrentLand(currentId))
    }, [])
    useEffect(() => {
    ...
    const [table, setTable] = useState('land')
        ...
        if (realImages.length === 0 && images.length === 0) {
            ...
            try {
                for (let i = 0; i < editImages.length; i++) {
                    ...
                    imgformdt.append('type', 'LandAreas')
                ...
                await axios.post(`${baseUrl}/landareas/update`, formdata, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    }
                })
            ...
            {((currentFlat?.[0]?.[0]?.type === 'Sale' && Admins?.[0]?.type !== 'daily') || ((currentFlat?.[0]?.[0]?.type === 'For Rent' || currentFlat?.[0]?.[0]?.type === 'Short Term') && Admins?.[0]?.type !== 'seller')) && <Box >
                <form onSubmit={handleSubmit}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        {currentFlat?.[0]?.[0]?.active == 1 && <a href={`https://www.yerevanhouse.am/${currentFlat?.[0]?.[0]?.type?.toLowerCase() === 'sale' ? 'sale' : 'daily'}/land/${currentFlat?.[0]?.[0]?.id}`} target='_blank' style={{ color: 'black' }}>{`https://www.yerevanhouse.am/${currentFlat?.[0]?.[0]?.type?.toLowerCase() === 'sale' ? 'sale' : 'daily'}/land/${currentFlat?.[0]?.[0]?.id}`}</a>}
                    </div>
                ...
                <DeleteModal open={openDelete} setOpen={setOpenDelete} id={currentId} action={LandDelete} />
            </Box>}
