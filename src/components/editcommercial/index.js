...
import { FlatPost, getCurrentFlat, resetFlatThunk } from "../../store/actions/flatAction";
...
import { CommercialDelete, getCurrentCommercial } from '../../store/actions/commercialAction';
...
const EditCommercial = () => {
    ...
    useEffect(() => {
        window.scrollTo(0, 0)
        dispatch(getCurrentCommercial(currentId))
    }, [])
    ...
    const [table, setTable] = useState('commerical')
    ...
    useEffect(() => {
        ...
        if (currentFlat?.[0]?.[0].type) {
            ...
            setMain({
                ...
                lead_area: currentFlat?.[0]?.[0].lead_area,
    ...
    const handleSubmit = async (e) => {
        ...
        if (realImages.length === 0 && images.length === 0) {
            ...
            //Apartments, LandAreas, Commercial, Houses
            try {
                for (let i = 0; i < editImages.length; i++) {
                    const imgformdt = new FormData()
                    imgformdt.append('id', editImages[i].id)
                    imgformdt.append('type', 'Commercial')
                ...
                await axios.post(`${baseUrl}/commercial/update`, formdata, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    }
                })
    ...
    const AllChecksMemo = useMemo(() => (
        <AllChecks table={table} type={turhtype} data={data2} setData={setData2} />
            ...
            {((currentFlat?.[0]?.[0]?.type === 'Sale' && Admins?.[0]?.type !== 'daily') || ((currentFlat?.[0]?.[0]?.type === 'For Rent' || currentFlat?.[0]?.[0]?.type === 'Short Term') && Admins?.[0]?.type !== 'seller')) && <Box >
                <form onSubmit={handleSubmit}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        {currentFlat?.[0]?.[0]?.active == 1 && <a href={`https://www.yerevanhouse.am/${currentFlat?.[0]?.[0]?.type?.toLowerCase() === 'sale' ? 'sale' : 'daily'}/commercial/${currentFlat?.[0]?.[0]?.id}`} target='_blank' style={{ color: 'black' }}>{`https://www.yerevanhouse.am/${currentFlat?.[0]?.[0]?.type?.toLowerCase() === 'sale' ? 'sale' : 'daily'}/commercial/${currentFlat?.[0]?.[0]?.id}`}</a>}
                    </div>
                ...
                <DeleteModal open={openDelete} setOpen={setOpenDelete} id={currentId} action={CommercialDelete} />
            </Box>}
