...
const HouseComponent = () => {
  ...
  const house = useSelector((state) => state.flatReducer.house);
  ...
  useEffect(() => {
    dispatch(resetFlatThunk());
    dispatch(getHouseThunk(page, par));
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    // if (String(filter.search).length > 0) {
    //   dispatch(resetFlatThunk())
    //   dispatch(getFlatSearchThunk(0, filter.search));
    // } else if (String(filter.byphone).length > 0) {
    //   dispatch(resetFlatThunk())
    //   dispatch(getFlatPhoneThunk(0, filter.byphone))
    // } else {
    let truth = {};
    let istino = {};
    truth = {
    ...
    // setPages(0);
    // dispatch(getHouseThunk(0, istino));
    const queryString = new URLSearchParams(istino).toString()
    navigate(`/privatehouses/1/` + new URLSearchParams(queryString).toString())
  ...
  const handlePageClick = (e) => {
    ...
    navigate(`/privatehouses/${Number(e.selected) + 1}/` + new URLSearchParams(queryString).toString())
  };
  ...
  const ProductMemo = useMemo(
    () => (
      <Product
        rows={house?.data}
        pages={Number(page) - 1}
        count={Math.ceil(house?.total / house?.per_page)}
          ...
          <div style={{ display: "flex" }}>
            ...
            >
              <FormControlLabel
                ...
                name="house"
                checked={tesak.house.toLowerCase() === 'house'.toLowerCase()}
                onChange={(e) => handleTesakChange(e, "house")}
              />
            </div>
            <div
              style={{
            ...
            >
              <FormControlLabel
                ...
                name="townhouse"
                checked={tesak.townhouse.toLowerCase() === 'townhouse'.toLowerCase()}
                onChange={(e) => handleTesakChange(e, "townhouse")}
              />
            </div>
            <div
              style={{
            ...
            >
              <FormControlLabel
                ...
                name="country_house"
                checked={tesak.country_house.toLowerCase() === 'country house'.toLowerCase()}
                onChange={(e) => handleTesakChange(e, "country house")}
              />
            </div>
          </div>
        </FormControl>
  ...
  return (
    <div>
      ...
      {codeflat === null && phoneflat === null && house.length === 0 && <Loading />}
      {codeflat !== null && CodeFlatMemo}
      {phoneflat !== null && PhoneFlatMemo}
      {house && phoneflat === null && ProductMemo}
    </div>
  );
};
