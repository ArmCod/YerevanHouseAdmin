...
import { FestivalDelete, FestivalPut, getFestivalThunk } from '../../store/actions/festivalAction';
import FestivalAddModal from './festivalAddModal';
...
const FestivalComponent = () => {
    ...
    const festival = useSelector(state => state.countryReducer.festival)
                ...
                {data && data.map((i, k) => (
                    <div style={{ marginBottom: 60 }} key={i.id}>
                        ...
                        <div style={{ display: 'flex', gap: 100 }}>
                            ...
                            <DeleteModal open={openDel} setOpen={setOpenDel} id={i.id} action={FestivalDelete} />
                        </div>
                    </div>))}
                <Button color="secondary" variant="contained" onClick={() => setOpenAdd(true)}>
                    Ավելացնել
                </Button>
                <FestivalAddModal openAdd={openAdd} setOpenAdd={setOpenAdd} />
