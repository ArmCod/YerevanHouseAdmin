...
const EditHouse = () => {
    ...
    useEffect(() => {
        window.scrollTo(0, 0)
        dispatch(getCurrentHouse(currentId))
    }, [])
    ...
    const [table, setTable] = useState('house')
    ...
    const handleSubmit = async (e) => {
        ...
        if (realImages.length === 0 && images.length === 0) {
            ...
            try {
                ...
                await axios.post(`${baseUrl}/house/update`, formdata, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    }
                })
            ...
            {((currentFlat?.[0]?.[0]?.type === 'Sale' && Admins?.[0]?.type !== 'daily') || ((currentFlat?.[0]?.[0]?.type === 'For Rent' || currentFlat?.[0]?.[0]?.type === 'Short Term') && Admins?.[0]?.type !== 'seller')) && <Box >
                <form onSubmit={handleSubmit}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        {currentFlat?.[0]?.[0]?.active == 1 && <a href={`https://www.yerevanhouse.am/${currentFlat?.[0]?.[0]?.type?.toLowerCase() === 'sale' ? 'sale' : 'daily'}/house/${currentFlat?.[0]?.[0]?.id}`} target='_blank' style={{ color: 'black' }}>{`https://www.yerevanhouse.am/${currentFlat?.[0]?.[0]?.type?.toLowerCase() === 'sale' ? 'sale' : 'daily'}/house/${currentFlat?.[0]?.[0]?.id}`}</a>}
                    </div>
                ...
                <DeleteModal open={openDelete} setOpen={setOpenDelete} id={currentId} action={HouseDelete} />
            </Box>}
